...
```{r}
MyMap <- WorldMap + 
  geom_point(aes(x=Long, y=Lat), data = EMCRlocationdata, colour = "#CF6B21", size =8, alpha=0.8, shape=17) +
  ylim(-65,90) +
...
```{r}
AusMap <- WorldMap + 
  geom_point(aes(x=Long, y=Lat), data = EMCRlocationdata, colour = "#CF6B21", size =12, alpha=0.6, shape=17) +
  ylim(-45,-11) +
  xlim(110, 155) +
  theme(axis.text=element_text(size=30),
        axis.title=element_text(size=45)) +
...
ggsave(plot=AusMap, device= "tiff", file='Outputs/ausmaplocations.tiff', width=18, height =12)
```
